...
* Given a Ruby version `A.B.C`, update the TruffleRuby version in `mx.truffleruby/suite.py` to `AB.C.0`.
* Reset `truffleruby-abi-version.h` to `$RUBY_VERSION.1` and `lib/cext/ABI_check.txt` to `1` if `RUBY_VERSION` was updated.
  * use `$TRUFFLERUBY_VERSION.1` for releases in `truffleruby-abi-version.h`
