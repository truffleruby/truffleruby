...
public final class ValueWrapper extends WeakReference<Object> implements TruffleObject {
     ...
     * fully usable, which means keeping the whole chain alive: the object, this wrapper and its {@link HandleBlock}
     * (without them the handle can no longer be resolved back to the object). For an object which strongly references
     * its wrapper back (RubyDynamicObject and ImmutableRubyObject), referencing the object is enough: it keeps this
     * wrapper alive, which keeps its block alive. A boxed Long/Double has no such back-reference, so referencing only
     * the box would let this wrapper and its block be collected, leaving a dead handle: this wrapper must be referenced
     * instead, and it keeps the box alive through {@link #strongRef}. For a tagged long there is nothing which needs to
     * stay alive and this wrapper is returned so callers never store null. */
