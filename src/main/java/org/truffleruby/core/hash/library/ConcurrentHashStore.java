...
public final class ConcurrentHashStore {
    ...
    // TODO have a mode to create all Hash as ConcurrentHashStore to run specs and test compatibility extensively
    ...
    private ConcurrentEntry getLastEntry() {
        var entry = tail.getPreviousInSequence();
        if (entry.isRemoved()) {
    ...
    public void getAdjacentObjects(Set<Object> reachable) {
        ...
        while (entry != tail) {
            if (!entry.isRemoved()) { // TODO need to check non-null key/removed as well?
    ...
    @TruffleBoundary
    private static void resize(RubyHash hash, int newSize) {
        System.err.println("RESIZE " + newSize);
        ConcurrentHashStore store = ConcurrentHashStore.getStore(hash);
        ...
        store.setBuckets(newEntries);
        assert store.verify(hash);
    }

    @TruffleBoundary
    private static void doResize(RubyHash hash, LightweightLayoutLock lock, int newSize, boolean takeLock) {
        final long stamp = takeLock ? lock.startLayoutChange() : 0;
        try {
    ...
    public static void copyIntoBuckets(RubyHash from, RubyHash to) {
        ConcurrentHashStore fromStore = ConcurrentHashStore.getStore(from);
        assert fromStore.verify(from);

        assert !(to.store instanceof ConcurrentHashStore) && !SharedObjects.isShared(to);
        assert HashStoreLibrary.verify(to);
        ...
        int size = from.getSizeVolatile();
        to.setStore(new BucketsHashStore(newEntries, firstInSequence, lastInSequence));
        to.size = size;
        assert HashStoreLibrary.verify(to);
    }

    @TruffleBoundary
    public static void convertFromOtherStrategy(RubyHash hash, int newCapacity) {
        int capacity = Math.max(newCapacity, INITIAL_CAPACITY);
    ...
    private static void fromOtherStrategyWithNewBuckets(RubyHash from, RubyHash to,
        ...
        to.setSizeVolatile(from.size);
    }

    public static Iterator<ConcurrentEntry> iterateEntries(RubyHash hash) {
        final ConcurrentHashStore concurrentHash = ConcurrentHashStore.getStore(hash);
    ...
    public static Iterable<ConcurrentEntry> iterableEntries(RubyHash hash) {
    ...
    abstract static class SetNode extends RubyBaseNode {
        ...
        @Specialization
        boolean set(RubyHash hash, Object originalKey, Object value, boolean byIdentity, boolean takeLock,
                @Bind Node node,
            ...
            while (true) {
                ...
                if (foundProfile.profile(node, entry == null)) {
                    ...
                    try {
                        boolean success;
                        var buckets = ConcurrentHashStore.getStore(hash).buckets;
                    ...
                    if (resizeProfile.profile(node, newSize * 4 > bucketsCount * 3)) {
                        doResize(hash, lock, newSize, takeLock);
                    }
    ...
    protected boolean set(RubyHash hash, Object originalKey, Object value, boolean byIdentity,
        ...
        return setNode.execute(hash, originalKey, value, byIdentity, true);
    }

    // TODO test if setStore assert triggers
    @ExportMessage
    ...
    private static Object removeEntry(RubyHash hash, ConcurrentEntry entry,
        ...
        boolean removed;
        var lock = ConcurrentHashStore.getStore(hash).lock;
        var fiber = RubyLanguage.get(node).getCurrentFiber();
        var threadState = fiber.getLayoutLockThreadState(lock, getThreadStateProfile, node);
        lock.startWrite(threadState, startWriteProfile, node);
        try {
            ...
            if (removed) {
                ...
                var store = ConcurrentHashStore.getStore(hash).buckets;
                removeFromLookup(hash, entry, store);
            }
        } finally {
            lock.finishWrite(threadState);
        }
    ...
    protected void replace(RubyHash hash, RubyHash dest) {
        // No need for PropagateSharingNode, we are copying from a shared Hash
        assert verify(hash);
        ...
        if (SharedObjects.isShared(dest)) {
            final long stamp = lock.startLayoutChange();
            try {
                var destStore = ConcurrentHashStore.getStore(dest);
                destStore.clearInsideLayoutChange(dest, buckets.length());

                fromOtherStrategyWithNewBuckets(hash, dest, this);
    ...
    protected void rehash(RubyHash hash,
        ...
        try {
            ...
            setBuckets(new AtomicReferenceArray<>(buckets.length()));
            hash.size = 0;
            var entry = getFirstEntry();

            while (entry != tail) {
                // FIXME: This is calling #hash inside a Layout Change, but is there any alternative?
                // NOTE: keeping the same ConcurrentEntry and mutating the hashed field doesn't work properly,
                // because multiple keys can rehash to the "same" key and deduplicate.
                setNode.execute(hash, entry.getKey(), entry.getValue(), hash.compareByIdentity, false);
                entry = nextNonRemovedEntry(entry);
            }
        } finally {
            lock.finishLayoutChange(stamp);
