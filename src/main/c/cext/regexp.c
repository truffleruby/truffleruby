...
VALUE rb_backref_get(void) {
  return rb_tr_up_rb_backref_get();
}

void rb_backref_set(VALUE str) {
  rb_tr_up_rb_backref_set(str);
}

VALUE rb_reg_match_pre(VALUE match) {
  return rb_tr_up_rb_reg_match_pre(match);
}

VALUE rb_reg_new(const char *s, long len, int options) {
  return rb_tr_up_rb_reg_new(rb_str_new(s, len), options);
}

VALUE rb_reg_compile(VALUE str, int options, const char *sourcefile, int sourceline) {
  // TODO BJF May-29-2020 implement sourcefile, sourceline
  return rb_tr_up_rb_reg_compile(str, options);
}

VALUE rb_reg_new_str(VALUE s, int options) {
  return rb_tr_up_rb_reg_new_str(s, options);
}

VALUE rb_reg_nth_match(int nth, VALUE match) {
  return rb_tr_up_rb_reg_nth_match(nth, match);
}

int rb_reg_options(VALUE re) {
  return FIX2INT(rb_tr_up_rb_reg_options(re));
}

VALUE rb_reg_regcomp(VALUE str) {
  return rb_tr_up_rb_reg_regcomp(str);
}

VALUE rb_reg_match(VALUE re, VALUE str) {
  return rb_tr_up_rb_reg_match(re, str);
}
