...
VALUE rb_tracepoint_new(VALUE target_thval, rb_event_flag_t events, void (*func)(VALUE, void *), void *data) {
  ...
  return rb_tr_up_rb_tracepoint_new(events, func, data);
}

VALUE rb_tracepoint_enable(VALUE tpval) {
  return rb_tr_up_send0_enable(tpval);
}

VALUE rb_tracepoint_disable(VALUE tpval) {
  return rb_tr_up_send0_disable(tpval);
}

VALUE rb_tracepoint_enabled_p(VALUE tpval) {
  return rb_tr_up_send0_enabled_p(tpval);
}
